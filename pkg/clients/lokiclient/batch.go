...
import (
	...
	"github.com/suikast42/logunifier/pkg/clients/lokiclient/api"
)
...
type batch struct {
	...
	maxStreams        int
	FeedbackNotifiers []api.FeedbackNotifier[any]
}
...
func newBatch(maxStreams int, entries ...api.Entry) *batch {
	b := &batch{
		...
		FeedbackNotifiers: make([]api.FeedbackNotifier[any], 0),
	}
...
func (b *batch) add(entry api.Entry) error {
	...
	// Add feedback notifier
	if entry.FeedbackNotifier.SendCtx != nil {
		b.FeedbackNotifiers = append(b.FeedbackNotifiers, entry.FeedbackNotifier)
	}
