...
func (c *client) sendBatch(tenantID string, batch *batch) {
	...
	for {
		...
		if err != nil || status/100 != 2 {
			for _, notifier := range batch.FeedbackNotifiers {
				notifier.OnFailure(notifier.Entry, notifier.SendCtx, status, err)
			}
		} else {
			for _, notifier := range batch.FeedbackNotifiers {
				notifier.OnSuccess(notifier.Entry, notifier.SendCtx, status)
			}
		}
